...
class TestDataContext extends Data.EventBase<Data.Change, Data.ChangeEvents>
  ...
  elementInserted(
      owner: TestDataContextObject, prop: Data.ChildPropertyTypes, index: number) : void {
    const value = prop.get(owner).get(index);
    this.elementInsert = { owner, prop, index, value };
    this.onElementInserted(owner, prop, index);
  }
  elementRemoved(
      owner: TestDataContextObject, prop: Data.ChildPropertyTypes, index: number, oldValue: TestDataContextObject) : void {
  ...
  private onElementInserted(
      item: TestDataContextObject, prop: Data.ChildPropertyTypes, index: number) {
    const change: Data.Change =
        { type: 'elementInserted', item: item, prop: prop, index: index, oldValue: undefined };
    const value = prop.get(item).get(index) as TestDataContextObject;
    value.parent = item;
    super.onEvent('elementInserted', change);
    return this.onChanged(change);
  }
  private onElementRemoved(
      item: TestDataContextObject, prop: Data.ChildPropertyTypes, index: number, oldValue: TestDataContextObject ) :
      Data.Change {
    const change: Data.Change =
...
class TestDataContextObject implements Data.DataContextObject, Data.ReferencedObject,
  ...
  get child() { return this.template.child.get(this) as Data.Slot<TestDataContextObject>; }
...
describe('Cloning', () => {
  test('copyItems', () => {
    ...
    const child1Copy = itemCopy.array.get(0) as TestDataContextObject,
          child2Copy = itemCopy.array.get(1) as TestDataContextObject;
...
describe('Serialization, deserialization', () => {
  test('serialize/deserialize', () => {
    ...
    expect(itemCopy.array.get(0) instanceof TestDataContextObject).toBe(true);
    const child3Copy = itemCopy.array.get(0) as TestDataContextObject;
    expect(child3Copy.array.length).toBe(2);
    expect(child3Copy.x).toBe(3);
    const child1Copy = child3Copy.array.get(0) as TestDataContextObject,
          child2Copy = child3Copy.array.get(1) as TestDataContextObject;
