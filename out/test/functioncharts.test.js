...
describe('Type', () => {
    ...
    test('base Types', () => {
        const valueType = FC.Type.fromString('v'), emptyType = FC.Type.fromString('[,]');
        expect(valueType).toBe(FC.Type.valueType);
        expect(emptyType).toBe(FC.Type.emptyType);
    });
    test('atomized', () => {
        const valueType = FC.Type.valueType, type1 = FC.Type.fromInfo([new FC.Pin(valueType), new FC.Pin(valueType)], [new FC.Pin(valueType)]), type2 = FC.Type.fromInfo([new FC.Pin(valueType), new FC.Pin(valueType)], [new FC.Pin(valueType)]);
...
describe('FunctionchartContext', () => {
    ...
    test('element typeString', () => {
        const context = new FC.FunctionchartContext(), functionchart = context.root, element = addElement(functionchart, '[,]');
    ...
    test('recursiveFunctionchart', () => {
        ...
        const fc = functionchart.nodes.at(0);
        expect(fc).toBeInstanceOf(FC.Functionchart);
        expect(fc.nodes.length).toBe(8);
        const rfi1 = fc.nodes.at(6), rfi2 = fc.nodes.at(7);
    ...
    test('canAddItem', () => {
        ...
        context.root = functionchart;
        const greatGrandparent = functionchart.nodes.at(0), grandParent = greatGrandparent.nodes.at(0), fc1 = grandParent.nodes.at(0), fc2 = grandParent.nodes.at(1);
